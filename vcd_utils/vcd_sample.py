...
def sample(
    ...
    **model_kwargs,
    ...
    model_kwargs_cd_multi = model_kwargs.copy()
    ...
    while True:
        ...
        use_multi_cd = model_kwargs.get("images_cd2") != None
        ...
        # if use_cd and not use_multi_cd:
        if use_cd:
            ...
            print(outputs_cd.keys())
            print
            num_layers = len(outputs_cd.past_key_values)
            print(f"Number of layers: {num_layers}")
            first_layer_key_shape = outputs_cd.past_key_values[0][0].shape
            first_layer_value_shape = outputs_cd.past_key_values[0][1].shape
            print(f"First layer key shape: {first_layer_key_shape}")
            print(f"First layer value shape: {first_layer_value_shape}")
            ...
            model_inputs_multi_cd = self.prepare_inputs_for_generation_cd_multi(input_ids, **model_kwargs_cd_multi)
            outputs_multi_cd = self(
                **model_inputs_multi_cd,
                return_dict=True,
                output_attentions=output_attentions_wo_img,
                output_hidden_states=output_hidden_states_wo_img,
            )
            ...
            next_token_logits_cd = outputs_cd.logits[:, -1, :]
            next_token_logits_multi_cd = outputs_multi_cd.logits[:, -1, :]
            ...
            avg_next_token_logits_cd = (next_token_logits_cd + next_token_logits_multi_cd) / 2

            diffs = (1 + cd_alpha) * next_token_logits - cd_alpha * avg_next_token_logits_cd
            cd_logits = diffs.masked_fill(next_token_logits < cutoff, -float("inf"))
        ...
        if return_dict_in_generate:
            if output_scores:
                ...
                scores += ({"scores_cd": next_token_scores, "logits": next_token_logits},)
        ...
        if use_multi_cd:
            ...
            model_kwargs_cd_multi = self._update_model_kwargs_for_generation(
                outputs_multi_cd, model_kwargs_cd_multi, is_encoder_decoder=self.config.is_encoder_decoder
            )
